...
if (!empty($_POST) && wp_verify_nonce($_REQUEST['_wpnonce'], 'mxp-fb2wp-main-setting-page')) {
	...
	if (has_shortcode($post_footer, 'mxp_fb2wp_display_embed')) {
		//待處理
		echo "<script>alert('" . __('Do not contain shortcode [mxp_fb2wp_display_embed] in footer contents for synced posts.', 'fb2wp-integration-tools') . "');</script>"; //「轉發文章Footer內容」中 請勿包含「mxp_fb2wp_display_embed」shortcode
		unset($post_footer);
	}
}
...
<div class="sidebar bar-block">
  ...
  <a href="javascript:void(0)" data-id="fbapp" class="<?php echo $tabs['fbapp']; ?>"><?php esc_html_e('Facebook App Settings', 'fb2wp-integration-tools'); //Facebook App 設定?></a>
  ...
  <a href="javascript:void(0)" data-id="messenger" class="<?php echo $tabs['messenger']; ?>"><?php esc_html_e('Facebook Messenger Bot Setup', 'fb2wp-integration-tools'); //Facebook 自動回覆設定?></a>
  <a href="javascript:void(0)" data-id="post_to_wp" class="<?php echo $tabs['post_to_wp']; ?>"><?php esc_html_e('Sync Posts to WordPress', 'fb2wp-integration-tools'); //文章同步回 WordPress 設定?></a>
  <a href="javascript:void(0)" data-id="fb_plugin" class="<?php echo $tabs['fb_plugin']; ?>"><?php esc_html_e('Facebook Plugins', 'fb2wp-integration-tools'); //Facebook 外掛功能?></a>
  <a href="javascript:void(0)" data-id="fb_ratings" class="<?php echo $tabs['fb_ratings']; ?>"><?php esc_html_e('Facebook Page Ratings', 'fb2wp-integration-tools'); //Facebook 粉絲頁評價?></a>
  <a href="javascript:void(0)" data-id="developer_function" class="<?php echo $tabs['developer_function']; ?>"><?php esc_html_e('Developer Tools', 'fb2wp-integration-tools'); //開發者功能?></a>
</div>
...
printf('<a href="%s" target="_blank">Facebook App</a> ' . esc_html__('Settings', 'fb2wp-integration-tools') . '</h2>', 'https://developers.facebook.com');?></h3>
		...
		<p><?php esc_html_e('Enable Facebook JavaScript SDK: ', 'fb2wp-integration-tools'); //啟用 Facebook JavaScript SDK：?>
...
esc_html_e('SDK Locale: ', 'fb2wp-integration-tools'); //設定 SDK 語言：?>
		<?php
...
esc_html_e('SDK Version: ', 'fb2wp-integration-tools'); //設定 SDK 版本 ?>
...
<div id="webhooks" class="container Section">
		...
		<p><?php esc_html_e('Please fill in the following URL to Callback URL in Webhooks settings', 'fb2wp-integration-tools'); //請將下列連結填入 App 應用程式 Webhooks 分頁中 Page 粉絲頁訂閱之回呼網址?></p>
		<p>
...
esc_html_e('Verify Token: ', 'fb2wp-integration-tools'); //回呼驗證權杖：?>
...
<div id="messenger" class="container Section">
		<h3><?php esc_html_e('Facebook Messenger Bot Setup', 'fb2wp-integration-tools'); //Facebook 自動回覆設定?></h3>
		<p><?php esc_html_e('Enable Messenger Bot: ', 'fb2wp-integration-tools'); //是否啟用：?>
...
esc_html_e('Default reply: ', 'fb2wp-integration-tools'); //若無比對到訊息時的預設回覆：?>
		<textarea name="mxp_messenger_default_reply" rows="3" cols="30"><?php echo get_option("mxp_messenger_default_reply"); ?></textarea><br /><?php
esc_html_e('You may use [mxp_input_msg] as a placeholder of users input message. For example, "Oops, I don\'t understand what you mean by saying \'[mxp_input_msg]\'. Please try other commands."', 'fb2wp-integration-tools');
...
//是否啟用<a href="https://developers.facebook.com/docs/messenger-platform/handover-protocol" target="_blank">交接模式</a>切換按鈕：?>
		...
		<p><?php _e('Text shown on Handover Protocol switch button: ', 'fb2wp-integration-tools'); //交接模式切換按鈕顯示文字：?>
...
echo get_option("mxp_fb2wp_messenger_enable_pass_thread_btn_text",
	...
	esc_html__('Click here to inform the admin.', 'fb2wp-integration-tools')); //點擊此處後留言通知管理員
 ?>" />
		</p>
</div>
<div id="post_to_wp" class="container Section">
		<h3><?php esc_html_e('Sync Posts to WordPress', 'fb2wp-integration-tools'); //文章同步回 WordPress 設定?></h3>
		<p><?php esc_html_e('Enable Sync Posts to WordPress', 'fb2wp-integration-tools'); //是否啟用：?>
...
esc_html_e('Post Author: ', 'fb2wp-integration-tools'); //發文使用者： ?>
...
esc_html_e('Post Category: ', 'fb2wp-integration-tools'); //發文分類：?>
...
esc_html_e('Post Status & Visibility: ', 'fb2wp-integration-tools'); //發文能見度狀態：?>
		...
		<p><?php esc_html_e('Allow Comments: ', 'fb2wp-integration-tools'); //允許迴響：?>
		...
		<p><?php esc_html_e('Allow Pingbacks & Trackbacks: ', 'fb2wp-integration-tools'); //允許通告： ?>
		...
		<p><?php esc_html_e('Post Type: ', 'fb2wp-integration-tools'); //文章類型：?>
		<?php
...
?>
		...
		<p><?php esc_html_e('Facebook accounts permitted to sync the posts: ', 'fb2wp-integration-tools'); //限定 Facebook 使用者投稿：?>
...
//（逗點（,）分隔 Facebook 用戶訊息發送 ID，不填入則表示所有人皆授權許可）?>
		...
		<p><?php esc_html_e('Post tags', 'fb2wp-integration-tools'); //發文標籤：?>
...
esc_html_e('Use comma-saparated format', 'fb2wp-integration-tools'); //（逗點（,）分隔）?>
		</p>
		<p><?php esc_html_e('Escape syncing posts with this hashtag: ', 'fb2wp-integration-tools'); //停止該篇轉發文章標籤：#?>#
		<input type="text" name="mxp_fb2wp_no_post_tag" size="10" value="<?php echo get_option("mxp_fb2wp_no_post_tag", ""); ?>" />
		(<?php esc_html_e('Do not append # to the defined hashtag.', 'fb2wp-integration-tools'); //（輸入不需加#號）?>)
		</p>
		<p><?php esc_html_e('Deafault title: ', 'fb2wp-integration-tools'); //替代標題：?>
...
//（文章抓取時使用FB發文的第一行視為標題，若第一行或內文為空，則帶入替代標題）?>)
		...
		<p><?php esc_html_e('Display attachments by default: ', 'fb2wp-integration-tools'); //預設顯示附件：?>
		...
		<p><?php esc_html_e('Display image captions by default: ', 'fb2wp-integration-tools'); //預設顯示圖片描述：?>
		...
		<p><?php esc_html_e('Display embedded posts by default: ', 'fb2wp-integration-tools'); //預設顯示嵌入文章：?>
		...
		<p><?php esc_html_e('Footer contents for synced posts (HTML, JavaScript, CSS and shortcodes supported): ', 'fb2wp-integration-tools'); //轉發文章Footer內容：（支援 HTML, JavaScript, CSS and shortcode）?>
		<textarea name="mxp_fb2wp_post_footer" rows="3" cols="40"><?php echo get_option("mxp_fb2wp_post_footer", ""); ?></textarea>
		</p>
		<h2><?php esc_html_e('Sync Comments Setup', 'fb2wp-integration-tools'); //留言同步回 WordPress 文章設定?></h2>
		<p><?php esc_html_e('To sync comments of posts shared to Facebook page back to the original posts.', 'fb2wp-integration-tools'); //文章分享至粉絲頁時，同步粉絲留言內容回網站該篇文章。?></p>
		<p><?php esc_html_e('Sync comments back: ', 'fb2wp-integration-tools'); //是否啟用?>
		...
		<p><?php esc_html_e('Comments approved by default: ', 'fb2wp-integration-tools'); //留言預設發佈：代表核准？?>
...
<div id="fb_plugin" class="container Section">
		<h2><?php esc_html_e('Facebook Plugins', 'fb2wp-integration-tools'); //Facebook 外掛功能?></h2>
		<p><?php esc_html_e('Block title: ', 'fb2wp-integration-tools'); //區塊標題：?>
		<input type="text" name="mxp_fb_functions_section_title" value="<?php echo get_option("mxp_fb_functions_section_title", esc_attr_e('<h3>Facebook Plugins</h3>', 'fb2wp-integration-tools')); ?>">
		<?php esc_html_e('(HTML, JavaScript and CSS supoorted)', 'fb2wp-integration-tools'); //（支援 HTML, JavaScript and CSS)?>
		</p>
		<p><?php esc_html_e('Enable Quote Plugin: ', 'fb2wp-integration-tools'); //啟用文章引言分享：?>
		...
		<p><?php esc_html_e('Enable Save Button', 'fb2wp-integration-tools'); //啟用文章儲存：?>
		...
		<p><?php esc_html_e('Enable Embedded Comments: ', 'fb2wp-integration-tools'); //啟用文章留言：待確認?>
		...
		<p><?php esc_html_e('Enable Customer Chat Plugin', 'fb2wp-integration-tools'); //網站嵌入粉絲頁 Messenger 顧客聊天：?>
		<input type="radio" name="mxp_fb_messenger_embed" value="show" <?php checked('show', get_option("mxp_fb_messenger_embed", "fade"));?>><label><?php esc_html_e('Show the dialog', 'fb2wp-integration-tools'); //展開對話框 ?></label>
		<input type="radio" name="mxp_fb_messenger_embed" value="fade" <?php checked('fade', get_option("mxp_fb_messenger_embed", "fade"));?>><label><?php esc_html_e('Show the dialog with delay', 'fb2wp-integration-tools'); //延遲展開對話框 ?></label>
		<input type="radio" name="mxp_fb_messenger_embed" value="hide" <?php checked('hide', get_option("mxp_fb_messenger_embed", "fade"));?>><label><?php esc_html_e('Hide the dialog', 'fb2wp-integration-tools'); //關閉對話框 ?></label>
		...
		(<a href="https://goo.gl/zXU8rH" target="_blank"><?php esc_html_e('How to embed Messenger Messenger Bot with FB2WP (Chinese)', 'fb2wp-integration-tools'); //參考使用 FB2WP 將臉書 Messenger 聊天機器人加入 WordPress 網站教學?></a>)
		<p><?php esc_html_e('Delays of showing the dialog (sec): ', 'fb2wp-integration-tools'); //Messenger 顧客聊天延遲展開對話框秒數：?>
		...
		<p><?php esc_html_e('Logged in greeting: ', 'fb2wp-integration-tools'); //Messenger 顧客聊天登入狀態顯示訊息：?>
			<input type="text" name="mxp_fb_messenger_logged_in_greeting" value="<?php echo get_option("mxp_fb_messenger_logged_in_greeting", esc_attr_e('Hello! How can we help you?', 'fb2wp-integration-tools')); ?>" size="40"/><br /><?php esc_html_e('(Maximum 80 characters)', 'fb2wp-integration-tools'); //（最多 80 字元）?>
		</p>
		<p><?php esc_html_e('Logged out greeting: ', 'fb2wp-integration-tools'); //Messenger 顧客聊天登出狀態顯示訊息：?>
			<input type="text" name="mxp_fb_messenger_logged_out_greeting" value="<?php echo get_option("mxp_fb_messenger_logged_out_greeting", esc_attr_e('Hello! How can we help you?', 'fb2wp-integration-tools')); ?>" size="40"/><br /><?php esc_html_e('(Maximum 80 characters)', 'fb2wp-integration-tools'); //（最多 80 字元）?>
		</p>
		<p><?php esc_html_e('Theme color: ', 'fb2wp-integration-tools'); //Messenger 顧客聊天主題顏色：?>
			...
			<?php esc_html_e('Hexadecimal color code without #', 'fb2wp-integration-tools'); //（HEX 色碼格式，無需 # 字號）?>
		</p>
		</p>
		<p><?php esc_html_e('Placement of Facebook Plugins', 'fb2wp-integration-tools'); //外掛區塊顯示位置：?>
		<input type="radio" name="mxp_fb_widget_place" value="up" <?php checked('up', get_option("mxp_fb_widget_place", "down"));?>><label><?php esc_html_e('Before the content', 'fb2wp-integration-tools'); //內容上方?></label>
		<input type="radio" name="mxp_fb_widget_place" value="down" <?php checked('down', get_option("mxp_fb_widget_place", "down"));?>><label><?php esc_html_e('After the content', 'fb2wp-integration-tools'); //內容下方?></label>
		</p>
		<p><?php esc_html_e('Clear Facebook URL Cache after updating posts', 'fb2wp-integration-tools'); //更新文章時同步清除 Facebook 快取：?>
...
<div id="fb_ratings" class="container Section">
	<h3><?php esc_html_e('Facebook Page Ratings', 'fb2wp-integration-tools'); //Facebook 粉絲頁評價?></h3>
	<p><?php
esc_html_e('1. Please subscribe to "ratings" event in your Facebook App before start using Facebook Page Ratings feature.', 'fb2wp-integration-tools');
//1. 使用粉絲頁評價同步功能前，請先記得於 Facebook App 中增加 「ratings」 的 Webhooks 事件訂閱。?></p>
	<p><?php
esc_html_e('2. Use shortcode [mxp_fb2wp_display_ratings] to display synced reviews from Facebook wherever on your site.', 'fb2wp-integration-tools');
//2. 前端輸出同步回網站的評價請使用「[mxp_fb2wp_display_ratings]」短碼，安插在任何希望呈現的角落。?></p>
	<p><?php
esc_html_e('3. The shortcode attribute "limit" will load 20 reviews by default. You may define other value, but too many requests may cause unexpected issues. Use "uid" to select certain user\'s review. Set "display_embed" attribute to "yes" to display the review in embed mode.', 'fb2wp-integration-tools');
//3. 短碼參數「limit」預設讀取 20 筆評價，可隨意指定數量（數值不建議過大，會有效能問題），「uid」指定某位 Facebook 使用者評價，最後是「display_embed」輸入「yes」值則會顯示該篇評價嵌入模式。?></p>
	<p><?php
esc_html_e('4. Developers may customize "fb2wp_display_ratings" to redefine the structure for better theme design.', 'fb2wp-integration-tools');
//4. 樣式可藉由「fb2wp_display_ratings」這個事件由開發者重新自由定義顯示結構，以符合主題設計。
?></p>
	<p><?php
esc_html_e('5. Press the following button to import current ratings of your Facebook Page (at most 100 reviews)', 'fb2wp-integration-tools');
...
<div id="developer_function" class="container Section">
		<h3><?php esc_html_e('Developer Tools', 'fb2wp-integration-tools'); //開發者功能?></h3>
		<p><?php esc_html_e('Delete the plugin as well as all settings: ', 'fb2wp-integration-tools'); //刪除外掛時連帶全部設定資料：?>
		...
		<p><?php esc_html_e('Enable saving log files: ', 'fb2wp-integration-tools'); //Log文件記錄：?>
		...
		<p> <?php esc_html_e('Eliminate current logs: ', 'fb2wp-integration-tools'); //清除目前記錄檔案：?>
		<input type="radio" name="mxp_remove_plugin_debug_log" value="yes"><label><?php esc_html_e('Enable', 'fb2wp-integration-tools');?></label>
		</p>
		<p><?php esc_html_e('Current logs: ', 'fb2wp-integration-tools'); //目前記錄檔：?>
		<?php
...
<p><?php esc_html_e('Donate developer: ', 'fb2wp-integration-tools');?><a href="https://goo.gl/XQYSq1" target="blank"><?php esc_html_e('Do you think the plugin helpful? Buy me a cup of coffee!', 'fb2wp-integration-tools'); //覺得有幫助嗎？請作者喝一杯咖啡吧！?></a></p>