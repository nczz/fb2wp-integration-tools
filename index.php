...
class Mxp_FB2WP {
    ...
    public static function register_facebook_save($content) {
        ...
        if (get_option("mxp_fb_save_enable", "yes") == "yes" || get_option("mxp_fb_save_enable", "yes") == "yes1") {
            ...
            $func .= '<p><div class="fb-save" data-size="' . $size . '" data-uri="' . $permalink . '"></div></p>';
        }
        // v1.5.0 新增 Facebook 小工具擺放位置的選項：文章內容上方、文章內容下方
        if (get_option("mxp_fb_widget_place", "down") == "down") {
            return $content . "<div id='mxp_fb_functions_section'>" . base64_decode(get_option("mxp_fb_functions_section_title", base64_encode(__('<h3>Facebook Plugins</h3>', 'fb2wp-integration-tools')))) . $func . "</div>";
        } else {
            return "<div id='mxp_fb_functions_section'>" . base64_decode(get_option("mxp_fb_functions_section_title", base64_encode("</h3>" . __('Facebook features:', 'fb2wp-integration-tools') . "</h3>"))) . $func . "</div>" . $content;
        }
    }
    ...
    public static function setting_fb_sdk() {
      ...
      window.fbAsyncInit = function() {
        FB.init({
          ...
          version    : '<?php echo get_option("mxp_fb_api_version", "v7.0"); ?>'
        });
      };
