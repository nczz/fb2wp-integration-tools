...
1. 粉絲頁內容同步回 WordPress 網站（ 「FB / 臉書」 -> WordPress）
2. 粉絲頁訊息機器人自動回覆功能
3. 粉絲頁評價同步回 WordPress 網站
4.  「FB / 臉書」 外掛：文章留言模組、網站粉絲頁 Messenger 顧客聊天整合、文章收藏
5. 發文自動更新  「FB / 臉書」 暫存快取
6. 支援  「FB / 臉書」 Webhooks 事件程式化：留言捕捉、心情按讚事件捕捉
7. 支援指定 SDK 語言和版本使用
...
1. ** 「FB / 臉書」 App設定** - 設定一組 App 資訊，作為網站端與  「FB / 臉書」 溝通的橋樑。

2. ** 「FB / 臉書」 Webhook設定** -  「FB / 臉書」 與 網站 端溝通的連結。

3. **Messenger 自動回覆設定** - 針對 Messenger 應答機器人的全域設定。

4. **文章同步設定** - 設定粉絲頁這邊通知網站有新文章時的相關設定。

5. ** 「FB / 臉書」 外掛功能** - 網站與  「FB / 臉書」 的相關小外掛整合功能設定處。

6. ** 「FB / 臉書」 粉絲頁評價** - 匯入過去用戶對粉絲頁的評價至網站，往後可以透過外掛呼叫內容並使用。
...
- 文章引言 ：啟動全站內容將可以被瀏覽者重點文字選取後分享於  「FB / 臉書」 中 
- 文章儲存 ：儲存該篇文章於  「FB / 臉書」 ，可在[我的珍藏](https://www.facebook.com/saved/)裡找到
- 文章留言 ：網站文章或其他類型內容下方的留言功能，有分兩種模式，共存模式即為與原本留言功能共存，單一模式為僅保留  「FB / 臉書」 留言功能。
- Messenger 顧客聊天 ：全站頁面右下角嵌入  「FB / 臉書」 Messenger 聊天外掛

= 粉絲頁過去的文章怎麼辦，能同步回網站嗎？ =

這部分很遺憾外掛只能做到「從安裝外掛啟用之後」的文章，粉絲頁上舊文章經過測試，可以使用這款「[ 「FB / 臉書」 Fanpage import](https://tw.wordpress.org/plugins/facebook-fanpage-import/)」外掛進行匯入！
...
當然可以，透過 `fb2wp_display_ratings` 事件，你將會得到一個原始陣列資料，愛怎麼客製就怎麼客製化囉！ 記得善用短碼的參數 `uid` 可以設定顯示某個人的評價 `limit` 為預設顯示20筆評價 `display_embed` 設定為 `yes` 則能嵌入  「FB / 臉書」 的評價訊息哦！

= 網站上  「FB / 臉書」 留言外掛功能消失的除錯流程 =

外掛開發遵守系統設定，首先需要檢查文章是否設定為關閉留言（允許迴響被打勾），再來是確定FB工具箱設定中「 「FB / 臉書」 外掛功能」處的「啟用文章留言」功能是否被選擇為「否」，以上都確認無誤還是發生留言外掛無作用的問題，請聯絡作者我處理！（外掛這邊開詢問不會通知作者，效率較慢哦！）
...
* 修正  「FB / 臉書」 客戶聊天外掛替換 SDK 路徑

= 1.7.6 =

* 修正  「FB / 臉書」 JS SDK 留言外掛參數讀取頁面路徑為必填
...
* 依照  「FB / 臉書」 文件更新外掛規格，讓顧客聊天外掛功能完整！（可更新顯示字眼、設定顯示模式、樣式）
...
* 新增快取功能，解決重複訂閱或是  「FB / 臉書」 重複發送請求導致匯入重複內容方法
* 新增  「FB / 臉書」 發文轉貼文章，留言同步回網站功能
* 更新常見問題之「 「FB / 臉書」 留言工具消失除錯方法」
...
* 新增  「FB / 臉書」 [顧客聊天外掛](https://developers.facebook.com/docs/messenger-platform/reference/web-plugins/#customer_chat)功能
...
* 更新  「FB / 臉書」 Messenger Platform 2.2 明年五月必須更新的 `messaging_type` 條件
* 調整架構為適合多訊息回覆模式，最大彈性支援回覆訊息種類與方法

= 1.5.4 =
* 修正方法引用的寫法避免警告出現
* 新增發文或更新文章時同步清除  「FB / 臉書」 快取的功能，有更新文章時就不用擔心臉書還在快取舊文了！
...
* 新增  「FB / 臉書」 小工具擺放位置的選項：文章內容上方、文章內容下方

= 1.4.9 =
* 新增 `fb2wp_comment_event` 事件方法，讓粉絲頁留言也能捕捉，使開發者能藉此方法建立自動回覆機制（註：要自動回覆需要 publish_pages, manage_pages 權限）
* 移除  「FB / 臉書」 太久都沒修好的「語系」資料，改為手動！！！
* 更新後端請求 FB 的 API 版本為 `v2.10`

= 1.4.8 =
* 修正 DEBUG 模式下顯示的錯誤資訊
* 因應  「FB / 臉書」 Webhooks 這次[故障](https://developers.facebook.com/bugs/463793280620151/)與[語系文件遺失](https://developers.facebook.com/bugs/1836827343245862/)補上追蹤原始請求紀錄和錯誤判斷
